...
public class BMTrPositionFragment extends BasePullFragment<BaseFragentPullDelegate, BaseFragmentPullBinder> {
                        ...
                        UiHeplUtils.shareImgViewBySdk(getActivity(),
                                share(
                                        ...
                                        new BigDecimal(adapter.getDatas().get(p).getTotalAmount()).doubleValue() >= 0 ?
                                                "多头 " + adapter.getDatas().get(p).getDetail().getLeverage() + "X" : "空头 " + adapter.getDatas().get(p).getDetail().getLeverage() + "X",
    ...
    private void initHeader() {
        ...
        View rootView2 = getActivity().getLayoutInflater().inflate(R.layout.layout_hole_position_ass, null);
        tv_balance = rootView2.findViewById(R.id.tv_balance);
        tv_assets = rootView2.findViewById(R.id.tv_assets);
        ...
        viewDelegate.viewHolder.fl_pull.addView(rootView2, 0);
    ...
    protected void onServiceSuccess(String data, String info, int status, int requestCode) {
        switch (requestCode) {
            case 0x123:
                ...
                break;
            case 0x124:
                tv_assets.setText(GsonUtil.getInstance().getValue(data, "btc"));
                tv_balance.setText(GsonUtil.getInstance().getValue(data, "btcAvailable"));
                break;
            case 0x125:
                //平仓
    ...
    protected void refreshData() {
        ...
        addRequest(binder.getCoinPosition(
                ...
                this));
        addRequest(binder.getBitmexDetail(this));
    }
}
